...
export const userQueries = {
  ...
  validateOtp: async (enteredOtp, emailForOtp, dispatch, navigation) => {
    try {
      ...
      if (snap.exists) {
        ...
        const decryptedBytes = CryptoJS.AES.decrypt(fetchedOtp, secretKey);
        ...
        if (enteredOtp === decryptedOTP) {
          const signInMethods = await auth().fetchSignInMethodsForEmail(
            emailForOtp
          ); // number of sign in methods for user
        ...
        // Snap does not exist
        return "Please terminate the app & try again.";
      }
    } catch (error) {
  ...
  setAvatar: async (userId, selectedAvatar) => {
    try {
      ...
      Sentry.captureException(error, {
        extra: {
          message: "Error while setting avatar from edit profile to firebase",
        },
      });
    }
  },
  ...
  setDeviceOnFirebase: async (deviceSelected, userId) => {
    try {
      ...
      Sentry.captureException(error, {
        extra: {
          message: "Error while setting the selected device on firebase",
        },
      });
    }
  },
  ...
  logoutUser: async (dispatch) => {
    try {
      ...
      // Alert the user if there is an error while logging the user out
