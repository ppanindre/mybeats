          ...
          >
            ...
            {errorMessage === "" && (
              <View>
                <Text style={{ fontSize: 16 }} className="text-orange-400">
                  ...
                  received the email, please check your spam folder or try
                  again.
                </Text>
              </View>
            )}
            ...
            <View style={{ width: 325 }}>
              <CustomButton
                ...
                btnLabel={
                  isLoading ? (
                    <ActivityIndicator color={customTheme.colors.light} />  // show activity indicator if the send otp function is loading
                  ) : resendCooldown === 0 ? (
                    "Resend One-Time-Password (OTP)"
                  ) : (
                    `Resend One-Time-Password (${resendCooldown} sec)` // show the timer for the resend cooldown
                  )
                }
                variant={resendCooldown === 0 ? "primary" : "disabled"} // if the resend cooldown is more than 0 then disable the button
              />
