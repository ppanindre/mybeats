...
const Dashboard = () => {
  ...
  const [lastModalTimestamp, setLastModalTimestamp] = useState(null);
  ...
  useEffect(() => {
    if (user.pointsAdded > 0) {
      const today = moment().startOf("day");
      const lastSync = user.lastSyncDateNew
        ? moment(user.lastSyncDateNew).startOf("day")
        : null;
      const diffDays = lastSync ? today.diff(lastSync, "days") : null;

      // only on first-ever sync, or exactly one day after lastSync
      if (!lastSync || diffDays === 1) {
  ...
  const updateStreak = async (userId) => {
    ...
    // Figure out new streak values
    let currentStreak = 1;
    let longestStreak = user.longestStreakNew || 1;

    if (user.lastSyncDateNew) {
      const lastSync = moment(user.lastSyncDateNew).startOf("day");
      const gap = moment().startOf("day").diff(lastSync, "days");

      if (gap === 1) {
        currentStreak = (user.currentStreakNew || 1) + 1;
        longestStreak = Math.max(longestStreak, currentStreak);
      }
    }

    // Optimistically update local state (so UI shows the increment immediately)
    setUser((prev) => ({
      ...prev,
      currentStreakNew: currentStreak,
      longestStreakNew: longestStreak,
      lastSyncDateNew: todayIso,
    }));
  ...
  const setPointsToZero = async () => {
    const userRef = firestore().collection("user").doc(user.userId);
    userRef.update({
      pointsAdded: 0,
    });
    ...
    >
      <View sentry-label="dashboard" className="p-5">
        <ScrollView ref={scrollViewRef} showsVerticalScrollIndicator={false}>
          {isModalVisible && (
            <SyncModal
              pointsAdded={user.pointsAdded || 0}
              points={user.todaysPoints?.todayScore || 0}
              onClose={setPointsToZero}
            />
          )}
          ...
          {isDashboardLoading ? (
            ...
            <View className="relative">
              {/* <View className="absolute right-0 flex-row space-x-1 items-center">
              ...
              {isVisible && latestNotification && notificationCount > 0 && (
                <View className="bg-white border-2 border-primary rounded-lg py-3 px-5 mt-5">
                  ...
                  {latestNotification?.type !== "Reminder" && (
