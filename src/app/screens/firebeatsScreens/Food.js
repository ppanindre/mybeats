...
const Food = () => {
    ...
    const fetchData = (forceFood = false) => {
        ...
        if (foodCache[dateFormatted] && !forceFood) {
            ...
            dispatch(FoodActionCreators.fetchData(dateFormatted)).then(
                (value) => {
                    setNutirentLoading(false);
                }
            );
    ...
    useEffect(() => {
        ...
        switch (incrementFormat) {
            ...
            case INCREMENT_MONTH:
                ...
                setChartLabels(
                    Array.from({ length: previous.daysInMonth() }).map(
                        (u, i) => i + 1
                    )
                );
                fetchFoodTrendDate(
    ...
    const fetchFoodTrendDate = (prevTime, nextTime) => {
        ...
        if (ids.length > 360) {
            ...
            chartData = Array.from({ length: ids.length }).map(() => [
                0, 0, 0, 0,
            ]);
        }
        ...
        fetchMultipleDcoumentsById(ids, USER_TABLE, {
            ...
            if (groupMonthly) {
                uniqueMonths.forEach((month) => {
                    chartData[month][0] =
                        chartData[month][0] /
                        months.filter((x) => x == month).length;
                    chartData[month][1] =
                        chartData[month][1] /
                        months.filter((x) => x == month).length;
                    chartData[month][2] =
    ...
    return (
        <View style={{ flex: 1, backgroundColor: "#fff" }}>
            ...
            {newDataAvailable && (
                ...
                >
                    <Text>
                        New data might be available , please pull down to
                        refresh
                    </Text>
                </View>
            )}
                        ...
                        {showEdit ? (
                            ...
                            <View className="p-5 justify-center">
                                ...
                                <FoodPieChart
                                    ...
                                    onSelectedSlice={(data) =>
                                        setSelectedSlice(data)
                                    }
                                    calories={
                                        foodTrendCard.length == 0
                                            ? 0
                                            : foodTrendCard[0]?.value ?? 0
                                    }
                                />
