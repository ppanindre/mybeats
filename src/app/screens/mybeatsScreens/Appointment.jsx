...
import { getDoctorForProfileView } from "../../../../store/actions/doctorActions";
...
const Appointment = ({ route }) => {
  ...
  const { loading: doctorLoading, doctor } = useSelector(
    (state) => state.doctorGetForProfileViewReducer
  );
  ...
  useEffect(() => {
    dispatch(getDoctorForProfileView(doctorId));
  }, []);

  console.log("doctor", doctorId, doctor);

  if (loading || doctorLoading || !doctor) return <Loader />;
    ...
    <ScreenContainer>
        ...
        <View className="space-y-5">
          ...
          <View>
            <AvailableAppointmentsFrame
              onSelectSlot={(slot) => selectAppointmentSlot("clinic", slot)}
          ...
          {doctor.availableForVideoConsultation && (
            <View className="space-y-5">
              ...
              <View>
                <AvailableAppointmentsFrame
                  onSelectSlot={(slot) => selectAppointmentSlot("video", slot)}
          ...
          <View className="flex-row justify-center mb-4">
            <Text className="font-semibol text-primary">View All Stories </Text>
            <Ionicons
      ...
      <View>
        <AppButton
          ...
          btnLabel="Book"
          btnLeftIcon={
            <Ionicons name="calendar" size={20} color={theme.colors.light} />
          }
        />
      </View>
    </ScreenContainer>
