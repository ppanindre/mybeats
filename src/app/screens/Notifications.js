...
const Notifications = () => {
  ...
  const fetchNotificationsForUser = async (appendToStart = false) => {
    ...
    notifications.forEach((data) => {
      // get date
      const date = moment(data.timestamp).format("MMM D, YYYY");
      ...
      if (updatedNotificationsData[date]) {
        updatedNotificationsData[date].push(data);
      } else {
    ...
    <CustomSafeView sentry-label="notification">
      <View className="relative flex-1">
        ...
        >
          ...
          {isLoading ? (
            <View className="justify-center" style={{ height: "100%" }}>
            ...
            <View>
              {displayNotificationsData !== null &&
                Object.keys(displayNotificationsData).map((date) => {
                  return (
                    // notification date
                    <View key={date}>
                      ...
                      {user.vendor !== "" &&
                        date ===
                          moment(user.joinDate).format("MMM DD, YYYY") && (
                          <View className="mb-6">
                            <View className="border-2 border-orange-400 p-3 rounded-lg">
                              <View className="flex-row space-x-1">
                                <HandThumbUpIcon color={customTheme.colors.primary} size={18} />
                                <Text>
                      ...
                      {displayNotificationsData[date].map((notify, index) => {
                        const show = showNotficationIndex === index; // if user selected show more, this should be true
                        const message = notify.message; // get notification message
                        const modified = message.replace(
                          "timestamp",
                          moment(notify.timestamp).format("HH:mm")
                        ); // modify the timestamp with different format

                        // detect should be true if notify tupe should be any of the following
                        const detect =
                          notify.type == "Afib" ||
                          notify.type == "Arrhythmia" ||
                          notify.type == "SLEEP" ||
                          notify.type == "Hypertension";
                        ...
                        return (
                          <View key={index} className="mb-6">
                            <View className="border-2 border-orange-400 p-3 rounded-lg">
                                  ...
                                  <View className="flex-row justify-end">
                                    <View>
                                      <TouchableOpacity
                                        ...
                                        onPress={() =>
                                          show
                                            ? setShowNotificationIndex(-1)
                                            : setShowNotificationIndex(index)
                                        }
                                      >
                                        ...
                                        {show ? (
                                          <ChevronUpIcon color={customTheme.colors.primary} />
                                        ) : (
                                          <ChevronDownIcon color={customTheme.colors.primary} />
                                        )}
                                      </TouchableOpacity>
                                    </View>
                                  </View>
                              ...
                              {show && (
                                <NotificationGraph notification={notify} />
                              )}
                            </View>
                          </View>
                        );
