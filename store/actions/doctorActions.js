  ...
  (doctorDetails, imageData) => async (dispatch, getState) => {
    try {
      ...
      const { secondarySpecializations } =
        getState().secondarySpecializationReducer;
      ...
      if (Array.isArray(doctorDetails.secondarySpecialization)) {
        ...
        secondarySpecializationIds = doctorDetails.secondarySpecialization
          .map((specName) => {
            const matchingSpec = secondarySpecializations.find(
        ...
        secondarySpecializationIds = doctorDetails.secondarySpecialization
          ...
          .map((specName) => {
            const matchingSpec = secondarySpecializations.find(
              (spec) => spec.name.trim() === specName.trim()
            );
            return matchingSpec ? matchingSpec.id : null;
          })
          .filter((id) => id !== null);
        ...
        variables: {
          input: {
            ...
            secondarySpecializationIds:
              secondarySpecializationIds.length > 0
                ? secondarySpecializationIds
                : null,
...
export const updateDoctorActionCreator =
  (doctorDetails, imageData, version) => async (dispatch, getState) => {
    try {
      ...
      let secondarySpecializationIds =
        doctorDetails.secondarySpecializationIds || [];
      if (Array.isArray(doctorDetails.secondarySpecialization)) {
        secondarySpecializationIds = doctorDetails.secondarySpecialization
          .map((specName) => {
            const matchingSpec = secondarySpecializations.find(
...
export const listDoctorsActionCreator = () => async (dispatch) => {
  try {
    ...
    //   "https://4u5ihc24qjh23eizmj36hak4ku.appsync-api.us-east-1.amazonaws.com/graphql",
    //   {
    //     method: "POST",
    //     headers: {
    //       "Content-Type": "application/json",
    //       "x-api-key": "da2-qabrh5d6urak3pdjqaftt27tmi",
    //     },
    //     body: JSON.stringify({
    //       query: `
    //         query ListDoctors {
...
export const getDoctorActionCreator = () => async (dispatch, getState) => {
  try {
    ...
    const mappedSecondarySpecializations = (
      doctorData.secondarySpecializationIds || []
    ).map((id) => {
      const matchedSpec = secondarySpecializations.find(
        (spec) => spec.id === id
      );
      return matchedSpec
        ? { id, name: matchedSpec.name }
        : { id, name: "Unknown" };
    });
